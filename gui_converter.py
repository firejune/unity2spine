...
class PreviewEngine:
    ...
    def load_bundle(self, src: Path) -> str:
        ...
        msg = (f"Bones: {len(bones)}  Slots: {len(slots)}  "
               f"Animations: {len(animations)}  Version: {SPINE_VERSION}")
        return msg
...
class PreviewOnlyWorker(QThread):
    ...
    def run(self):
        try:
            self.progress.emit(f"Loading: {self.src}")
            engine = PreviewEngine()
            msg = engine.load_bundle(self.src)
            self.progress.emit(f"Preview ready: {msg}")
            self.preview_ready.emit(engine)
            self.finished_signal.emit(True, "Preview loaded successfully")
        except Exception as e:
...
class ConvertWorker(QThread):
    ...
    def run(self):
        try:
            ...
            self.progress.emit(f"Exporting to: {out}")
            ...
            self.finished_signal.emit(True, f"Export successful: {out}")
        except Exception as e:
            self.progress.emit(f"Error: {e}")
            traceback.print_exc()
            self.finished_signal.emit(False, str(e))
...
class BatchConvertWorker(QThread):
    ...
    def run(self):
        ...
        for i, src in enumerate(self.sources):
            ...
            try:
                self.progress.emit(f"[{i + 1}/{total}] Processing: {src}")
                scene = load_scene(src)
                ...
                export_spine(scene, out, editor=self.editor)
                self.progress.emit(f"[{i + 1}/{total}] Done: {src.name} -> {folder_name}/")
                success += 1
            except Exception as e:
                self.progress.emit(f"[{i + 1}/{total}] Failed: {src} - {e}")
                fail += 1
...
class FolderScanWorker(QThread):
    ...
    def run(self):
        ...
        try:
            ...
            self.progress.emit(f"Error scanning folder: {e}")
            self.finished_signal.emit([], 0, 0)
            return

        total = len(all_files)
        self.progress.emit(f"Found {total} files. Detecting valid AssetBundles...")
        ...
        for i, fpath in enumerate(all_files):
            ...
            try:
                ...
                if has_renderer:
                    valid.append(fpath)
                    self.progress.emit(f"  ✓ [{i + 1}/{total}] Valid: {fpath.name}")
                else:
                    skipped += 1
            except Exception:
            ...
            if (i + 1) % 50 == 0:
                self.progress.emit(f"Scan progress: {i + 1}/{total} (Valid: {len(valid)}, Skipped: {skipped})")

        self.progress.emit(
            f"Scan complete: {total} files scanned, "
            f"found {len(valid)} valid AssetBundles, "
            f"skipped {skipped} non-bundle files"
        )
...
class PreviewCanvas(QOpenGLWidget):
    ...
    def paintEvent(self, event):
        ...
        if not self.engine:
            ...
            painter.drawText(QRectF(0, 0, w, h), Qt.AlignCenter, "Load an AssetBundle to preview")
            painter.end()
            return
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle(f"Unity AssetBundle → Spine {SPINE_VERSION} Converter")
    ...
    def _setup_ui(self):
        ...
        # File selection
        grp_file = QGroupBox("📁 Source File")
        fl = QVBoxLayout(grp_file)
        fl.setSpacing(4)

        fl_sel = QHBoxLayout()
        self.lbl_src = QLabel("No file selected")
        ...
        self.btn_batch_add = QPushButton("📦 Add Batch Files...")
        self.btn_batch_add.clicked.connect(self._browse_batch)
        fl.addWidget(self.btn_batch_add)

        self.btn_folder_scan = QPushButton("📁 Smart Scan Folder...")
        ...
        self.lbl_batch_count = QLabel("Batch: 0 files")
        ...
        self.btn_select_all.setMaximumWidth(70)
        ...
        br_layout.addWidget(QLabel("Prefix:"))
        self.txt_export_prefix = QLineEdit()
        self.txt_export_prefix.setPlaceholderText("Leave empty for original name")
        ...
        self.chk_use_stem_as_name = QCheckBox("Strip extension")
        ...
        # Output settings
        grp_out = QGroupBox("⚙ Export Settings")
        ol = QVBoxLayout(grp_out)
        ol.setSpacing(4)

        ol_mode = QHBoxLayout()
        ol_mode.addWidget(QLabel("Mode:"))
        ...
        ol_scale.addWidget(QLabel("Canvas Width:"))
        ...
        self.spin_target_w.setToolTip("Target canvas width (Spine scale)")
        ...
        self.btn_preview = QPushButton("👁 Preview Only (No Export)")
        ...
        # Convert button
        btn_row = QHBoxLayout()
        self.btn_convert = QPushButton("▶ Convert & Preview")
        ...
        self.btn_batch_convert = QPushButton("📦 Batch Export")
        ...
        self.chk_skeleton = QCheckBox("Bones")
    ...
    def dropEvent(self, event):
        ...
        if path.is_dir():
            ...
            self._log(f"Output directory: {self.folder_output_dir}")
        ...
        if len(urls) > 1:
            ...
            self.lbl_batch_count.setText(f"Batch: {len(self.batch_sources)} files")
            ...
            self._log(f"Added {len(self.batch_sources)} batch files")
        else:
    ...
    def _browse_src(self):
        path, _ = QFileDialog.getOpenFileName(
            self, "Select Unity AssetBundle", "",
            "AssetBundle (__data *);;All Files (*)"
        )
        if path:
            self.current_src = Path(path)
            self.lbl_src.setText(self.current_src.name)
            self.lbl_src.setStyleSheet("color:#e0e0e0; font-size:12px;")
            self._log(f"Selected file: {self.current_src}")

    def _browse_batch(self):
        paths, _ = QFileDialog.getOpenFileNames(
            self, "Select Multiple AssetBundle Files", "",
            "AssetBundle (__data *);;All Files (*)"
        )
        if paths:
    ...
    def _browse_folder_scan(self):
        ...
        folder = QFileDialog.getExistingDirectory(self, "Select Folder Containing AssetBundles")
        if not folder:
            return

        self.scanned_folder = Path(folder)
        self._log(f"Scanning folder: {self.scanned_folder}")
    ...
    def _on_folder_scan_done(self, valid_paths: list, skipped: int, errors: int):
        ...
        if not valid_paths:
            ...
            self._log("❌ No valid AssetBundle files found")
            QMessageBox.warning(self, "Scan Results", "No valid AssetBundle files found in selected folder.\n\nPlease verify that the folder contains Unity AssetBundles with SkinnedMeshRenderer or SpriteRenderer.")
            return

        self._log(f"✅ Found {len(valid_paths)} valid files, skipped {skipped} non-bundle files")
        ...
        self.lbl_batch_count.setText(f"Batch: {len(valid_paths)} files (skipped {skipped})")
        self.btn_batch_convert.setEnabled(True)
        self.batch_file_list.setVisible(True)
        self.batch_rename_bar.setVisible(True)
        self.btn_select_all.setVisible(True)
    ...
    def _start_folder_batch_export(self):
        ...
        if not checked:
            QMessageBox.warning(self, "Notice", "No files selected. Please check files in the list before exporting.")
            return

        if not self.folder_output_dir:
            QMessageBox.warning(self, "Notice", "Please select an output directory first.")
            return
        ...
        self._log(f"Starting batch export of {len(checked)} files...")
    ...
    def _browse_output(self):
        ...
        if path:
            ...
            self.lbl_output.setText("Default (same directory)")
            self.lbl_output.setStyleSheet("color:#808090; font-size:12px;")
    ...
    def _start_preview(self):
        ...
        if not self.current_src:
            QMessageBox.warning(self, "Notice", "Please select a source file first.")
            return

        if not self.current_src.exists():
            QMessageBox.critical(self, "Error", f"File does not exist: {self.current_src}")
            return
        ...
        self.txt_log.clear()
        self._log("Loading Bundle for preview...")
    ...
    def _on_preview_done(self, success: bool, msg: str):
        ...
        if success:
            self._log(f"✅ {msg}")
            self.lbl_info.setText("Preview ready (not exported)")
        else:
            self._log(f"❌ {msg}")
            self.lbl_info.setText("Preview failed to load")
    ...
    def _on_convert_done(self, success: bool, msg: str):
        ...
        if success:
            self._log(f"✅ {msg}")
            self.lbl_info.setText("Conversion complete")
        else:
            self._log(f"❌ {msg}")
            self.lbl_info.setText("Conversion failed")
    ...
    def _on_preview_ready(self, engine: PreviewEngine):
        ...
        self.lbl_info.setText(f"Bones: {len(engine.bones)}  Slots: {len(engine.skel['slots'])}  "
                              f"Animations: {len(engine.animations)}  Version: {SPINE_VERSION}")
    ...
    def _start_batch(self):
        ...
        if self.batch_file_list.isVisible():
            checked = self._get_checked_paths()
            if not checked:
                QMessageBox.warning(self, "Notice", "Please check files to export in the list, or use 'Smart Scan Folder'.")
                return
        elif not self.batch_sources:
            QMessageBox.warning(self, "Notice", "Please add batch files first.")
            return
        else:
        ...
        if out_base is None:
            ...
            self._log(f"Default output directory: {out_base}")
    ...
    def _on_batch_done(self, success: int, fail: int):
        self._enable_all_buttons()
        self.progress_bar.setVisible(False)
        self._log(f"✅ Batch export complete: {success} succeeded, {fail} failed")
        self.lbl_info.setText(f"Batch complete: {success}/{success + fail}")
